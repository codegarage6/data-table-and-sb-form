<template>
  ...
  <div class="offcanvas offcanvas-end w-50" tabindex="-1"
    ...
    <div class="offcanvas-header">
      ...
      <button type="button" id="file-modal-close-btn" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>

    <div class="offcanvas-body" v-if="show_files">
      <div class="row">
        <div class="col-md-3" v-if="markFiles.length > 0">
          <button class="btn btn-danger btn-sm" style="font-size: 12px" type="button" id="button-addon2" @click="deleteMarkedFile()">Delete Selected Item</button>
        </div>
        <div :class="`col-md-${markFiles.length > 0 ? '9' : '12'}`">
      ...
      <div class="d-flex align-content-start flex-wrap">
        <div v-for="file in file_list"
          ...
          <div class="pe-4 pt-2 position-absolute">
            <input type="checkbox" class="form-check-input" @change="markFile(file)" :ref="`file-${file.id}`">
          </div>
          ...
          <img :src="file.thumbnail" class="rounded w-100 h-100" :alt="file.name" @click="selectFile(file)">
          <p class="file-name py-2 rounded" @click="selectFile(file)">{{ file.name }}</p>
        </div>
...
export default {
  ...
  methods: {
    ...
    markFile(file) {
      const isExistFile = this.markFiles.indexOf(file.id);
      if (isExistFile === -1) {
        this.markFiles.push(file.id);
      } else {
        this.markFiles.splice(isExistFile, 1);
      }
    },
    removeFile(id) {
    ...
    deleteMarkedFile() {
      ...
      if (result) {
        ...
        this.axios.delete(`/files-delete-multiple/${this.markFiles.toString()}`, ).then(({data}) => {
          this.file_list = this.file_list.filter(item => !this.markFiles.includes(item.id));
          this.markFiles = [];
        }).catch(error => {
          throw error
        })
